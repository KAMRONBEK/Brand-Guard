...
export function MultiTabsProvider({ children }: { children: React.ReactNode }) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: only depend on currentRouteMeta.pathname
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: operations instance is recreated with tabs state
	useEffect(() => {
		if (!currentRouteMeta) return;
