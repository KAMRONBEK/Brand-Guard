...
function StatSparkBar({ color, data }: { color: string; data: number[] }) {
...
export default function Workbench() {
	...
	const overviewLineOptions = useChart({
		chart: { sparkline: { enabled: true } },
		colors: ["#ef4444"],
		grid: { show: false },
		yaxis: { show: false },
					...
					<Card key={stat.label} className="flex flex-col justify-between h-full">
						<CardContent className="flex flex-col gap-2 p-4">
							...
							<div className="w-full h-10 mt-2">
								<StatSparkBar color={stat.color} data={stat.chart} />
							</div>
						</CardContent>
					</Card>
				))}
